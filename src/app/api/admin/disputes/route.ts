...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await adminClient
      .from('disputes')
      .select(
        '*, order:orders(*, buyer:profiles!buyer_id(*), seller:profiles!seller_id(*)), opened_by:profiles!opened_by_id(*)',
        { count: 'exact' }
      )
    ...
    return errorResponse(message, 'Failed to fetch disputes', 500)
  }
}
